...
// Software entries live in the same `assets` table as hardware (type
// discriminator), just filtered here. Seats and license keys aren't
// tracked on this record -- that's what License Tracking is for.
export default function SoftwareInventory() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    const { data, error } = await supabase.rpc('get_assets', { p_type: 'software' });
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
        <Box>
          <Typography variant="h5">Software Inventory</Typography>
          <Typography variant="body2" color="text.secondary">
            Applications and platforms in use. License keys and seat counts live under License Tracking.
          </Typography>
        </Box>
      ...
      {loading ? (
        ...
        <Paper sx={{ p: 4, textAlign: 'center' }}>
          <Typography color="text.secondary">No software assets yet.</Typography>
        </Paper>
      ) : (
