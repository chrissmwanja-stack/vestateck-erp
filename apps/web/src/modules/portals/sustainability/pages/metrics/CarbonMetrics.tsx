...
export default function CarbonMetrics() {
  ...
  const fetchData = async () => {
    ...
    const [metricsRes, typesRes] = await Promise.all([
      supabase.from("sustainability_metrics").select("*, sustainability_metric_types(name, unit)").eq("type", "carbon").order("recorded_date", { ascending: false }).limit(100),
      supabase.from("sustainability_metric_types").select("id, name, unit").eq("type", "carbon").eq("is_active", true),
    ]);
    ...
    <Box sx={{ p: 3, maxWidth: 1100 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">Scope 1,2,3 emissions tracking. Unit tCO2e. Total {total.toLocaleString()} tCO2e, Avg {avg.toFixed(2)}.</Typography>
        </Box>
