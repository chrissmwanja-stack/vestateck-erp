...
export default function WasteMetrics() {
  ...
  const fetchData = async () => {
    ...
    const [metricsRes, typesRes] = await Promise.all([
      supabase.from("sustainability_metrics").select("*, sustainability_metric_types(name, unit)").eq("type", "waste").order("recorded_date", { ascending: false }).limit(200),
      supabase.from("sustainability_metric_types").select("id, name, unit").eq("type", "waste").eq("is_active", true).order("name"),
    ]);
    if (metricsRes.data) {
  ...
  const handleExcel = () => {
    const cols = [
      ...
      { header: "Unit", accessor: (r: any) => r.sustainability_metric_types?.unit || "kg" },
      { header: "Notes", accessor: (r: any) => r.notes || "-" },
    ];
    exportReportToExcel(`waste-metrics-${new Date().toISOString().slice(0,10)}`, "Waste", cols as any, metrics);
  };
  const handlePdf = () => {
    const cols = [
      { header: "Date", accessor: (r: any) => new Date(r.recorded_date).toLocaleDateString() },
    ...
    exportReportToPdf(`waste-metrics-${new Date().toISOString().slice(0,10)}.pdf`, "Waste Generated Report", cols as any, metrics, `Total ${total.toLocaleString()} kg • Avg ${avg.toFixed(2)} • ${metrics.length} records`);
  };
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1200 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: 2, gap: 2, flexWrap: "wrap" }}>
        <Box>
          <Typography variant="h5" fontWeight={700} gutterBottom>Waste Generated</Typography>
          <Typography variant="body2" color="text.secondary">Waste by category, diversion rate. Unit kg. Total {total.toLocaleString()} kg, Avg {avg.toFixed(2)}. Click edit to correct.</Typography>
        </Box>
      ...
      <Box sx={{ display: "flex", gap: 2, mb: 2, flexWrap: "wrap" }}>
        <Card sx={{ minWidth: 150 }}><CardContent><Typography variant="caption">Total Waste</Typography><Typography variant="h6" fontWeight={700}>{total.toLocaleString()} kg</Typography></CardContent></Card>
