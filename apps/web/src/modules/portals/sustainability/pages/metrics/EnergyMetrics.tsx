...
export default function EnergyMetrics() {
  ...
  const fetchData = async () => {
    ...
    const [metricsRes, typesRes] = await Promise.all([
      supabase.from("sustainability_metrics").select("*, sustainability_metric_types(name, unit)").eq("type", "energy").order("recorded_date", { ascending: false }).limit(200),
      supabase.from("sustainability_metric_types").select("id, name, unit").eq("type", "energy").eq("is_active", true).order("name"),
    ]);
    if (metricsRes.data) {
  ...
  const handleExcel = () => {
    const cols = [
      ...
      { header: "Unit", accessor: (r: any) => r.sustainability_metric_types?.unit || "kWh" },
      { header: "Notes", accessor: (r: any) => r.notes || "-" },
    ];
    exportReportToExcel(`energy-metrics-${new Date().toISOString().slice(0,10)}`, "Energy", cols as any, metrics);
  };
  const handlePdf = () => {
    const cols = [
      { header: "Date", accessor: (r: any) => new Date(r.recorded_date).toLocaleDateString() },
    ...
    exportReportToPdf(`energy-metrics-${new Date().toISOString().slice(0,10)}.pdf`, "Energy Consumption Report", cols as any, metrics, `Total ${total.toLocaleString()} kWh • Avg ${avg.toFixed(2)} • ${metrics.length} records`);
  };
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1200 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: 2, gap: 2, flexWrap: "wrap" }}>
        <Box>
          <Typography variant="h5" fontWeight={700} gutterBottom>Energy Consumption</Typography>
          <Typography variant="body2" color="text.secondary">Electricity, fuel consumption trends. Unit kWh. Total {total.toLocaleString()} kWh, Avg {avg.toFixed(2)}. Click edit to correct.</Typography>
        </Box>
      ...
      <Box sx={{ display: "flex", gap: 2, mb: 2, flexWrap: "wrap" }}>
        <Card sx={{ minWidth: 150 }}><CardContent><Typography variant="caption">Total Energy</Typography><Typography variant="h6" fontWeight={700}>{total.toLocaleString()} kWh</Typography></CardContent></Card>
