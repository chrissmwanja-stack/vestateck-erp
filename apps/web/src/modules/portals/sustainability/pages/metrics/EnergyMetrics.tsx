...
export default function EnergyMetrics() {
  ...
  const fetchData = async () => {
    ...
    const [metricsRes, typesRes] = await Promise.all([
      supabase.from("sustainability_metrics").select("*, sustainability_metric_types(name, unit)").eq("type", "energy").order("recorded_date", { ascending: false }).limit(100),
      supabase.from("sustainability_metric_types").select("id, name, unit").eq("type", "energy").eq("is_active", true),
    ]);
    ...
    <Box sx={{ p: 3, maxWidth: 1100 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">Electricity, fuel consumption trends. Unit kWh. Total {total.toLocaleString()} kWh, Avg {avg.toFixed(2)}.</Typography>
        </Box>
        <Button variant="contained" startIcon={<Add />} onClick={() => setOpen(true)}>Add Metric</Button>
      </Box>

      <Box sx={{ display: "flex", gap: 2, mb: 3 }}>
        <Card sx={{ minWidth: 150 }}><CardContent><Typography variant="caption">Total Carbon</Typography><Typography variant="h6" fontWeight={700}>{total.toLocaleString()} kWh</Typography></CardContent></Card>
