...
interface ContractType { id: string; tenant_id: string; name: string; description: string | null; is_active: boolean; }

export default function ContractTypesAdmin() {
  ...
  const fetchTypes = async () => {
    setLoading(true);
    const { data } = await supabase.from("law_contract_types").select("*").order("name");
  ...
  const handleOpenEdit = (t: ContractType) => { setEditing(t); setForm({ name: t.name, description: t.description || "", is_active: t.is_active }); setOpen(true); };

  const handleSave = async () => {
    if (!form.name.trim()) return;
    const payload = { name: form.name.trim(), description: form.description.trim() || null, is_active: form.is_active };
    if (editing) {
      const { error } = await supabase.from("law_contract_types").update(payload).eq("id", editing.id);
      ...
      const { error } = await supabase.from("law_contract_types").insert(insertPayload);
      if (error) { alert(error.message); return; }
    }
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Delete type? Contracts using it will block deletion.")) return;
    const { error } = await supabase.from("law_contract_types").delete().eq("id", id);
