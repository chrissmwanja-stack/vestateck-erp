...
interface CaseType { id: string; tenant_id: string; name: string; description: string | null; is_active: boolean; }
...
export default function CaseTypesAdmin() {
  ...
  const fetchTypes = async () => {
    setLoading(true);
    const { data } = await supabase.from("law_case_types").select("*").order("name");
  ...
  const handleOpenEdit = (t: CaseType) => { setEditing(t); setForm({ name: t.name, description: t.description || "", is_active: t.is_active }); setOpen(true); };

  const handleSave = async () => {
    if (!form.name.trim()) return;
    const payload = { name: form.name.trim(), description: form.description.trim() || null, is_active: form.is_active };
    if (editing) {
      const { error } = await supabase.from("law_case_types").update(payload).eq("id", editing.id);
      ...
      const { error } = await supabase.from("law_case_types").insert(insertPayload);
      if (error) { alert(error.message); return; }
    }
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Delete type? Cases using it will block deletion.")) return;
    const { error } = await supabase.from("law_case_types").delete().eq("id", id);
