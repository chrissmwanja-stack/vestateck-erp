...
interface Department { id: string; tenant_id: string; name: string; parent_id: string | null; is_active: boolean; created_at: string; }
...
export default function HRDepartmentsAdmin() {
  ...
  const handleOpenNew = () => { setEditing(null); setForm({ name: "", parent_id: "", is_active: true }); setOpen(true); };
  const handleOpenEdit = (d: Department) => { setEditing(d); setForm({ name: d.name, parent_id: d.parent_id || "", is_active: d.is_active }); setOpen(true); };

  const handleSave = async () => {
    if (!form.name.trim()) return;
    const payload: any = { name: form.name.trim(), parent_id: form.parent_id || null, is_active: form.is_active };
    if (editing) {
      const { error } = await supabase.from("hr_departments").update(payload).eq("id", editing.id);
