...
export default function NewProject() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: any = {
      ...
      // Status is deliberately not a form field since the approval flow
      // (20260921110000) landed: projects are created not_started and go
      // not_started -> submit -> pending_approval -> approved(in_progress)
      // from the detail page, instead of being born in whatever status the
      // picker happened to be on. created_by feeds the no-self-approval
      // rule in decide_pmo_project().
      status: "not_started",
