...
interface MachineType { id: string; tenant_id: string; name: string; description: string | null; is_active: boolean; }

export default function MachineTypesAdmin() {
  ...
  const fetchTypes = async () => {
    ...
    if (data) setTypes(data as MachineType[]);
    setLoading(false);
  };
  useEffect(() => { fetchTypes(); }, []);

  const handleOpenNew = () => { setEditing(null); setForm({ name: "", description: "", is_active: true }); setOpen(true); };
  const handleOpenEdit = (t: MachineType) => { setEditing(t); setForm({ name: t.name, description: t.description || "", is_active: t.is_active }); setOpen(true); };

  const handleSave = async () => {
    if (!form.name.trim()) return;
    const payload = { name: form.name.trim(), description: form.description.trim() || null, is_active: form.is_active };
    if (editing) {
      const { error } = await supabase.from("machine_types").update(payload).eq("id", editing.id);
      ...
      const { error } = await supabase.from("machine_types").insert(insertPayload);
      if (error) { alert(error.message); return; }
    }
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Delete type? Machines using it will block deletion.")) return;
