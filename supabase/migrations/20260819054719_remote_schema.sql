...
drop function if exists "public"."get_platform_dashboard_stats"();

drop function if exists "public"."get_tenant_workflow_stages"(p_tenant_id uuid);

drop function if exists "public"."update_workflow_stage_threshold"(p_stage_id uuid, p_threshold_amount numeric);
...
CREATE OR REPLACE FUNCTION public.get_companies_overview()
 RETURNS TABLE(tenant_id uuid, name text, status text, created_at timestamp with time zone, member_count bigint, module_count bigint, request_count_30d bigint, pending_request_count bigint)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
