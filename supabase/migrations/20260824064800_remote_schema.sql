...
drop policy "material_catalog_insert" on "public"."material_catalog";
...
drop function if exists "public"."is_business_dev_admin"();

drop function if exists "public"."is_it_support_admin"();
...
AS $function$
  SELECT COALESCE(
    (SELECT is_company_admin FROM app_users WHERE id = (SELECT auth.uid())),
    false
  );
$function$
;
...
CREATE OR REPLACE FUNCTION public.get_po_edit_history(po_id uuid)
 RETURNS TABLE(id uuid, edited_at timestamp with time zone, reason text, changes jsonb, editor jsonb)
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.record_invoice_approval_decision(p_invoice_request_id uuid, p_decision text, p_comment text DEFAULT NULL::text, p_acting_on_behalf_of uuid DEFAULT NULL::uuid)
...
CREATE OR REPLACE FUNCTION public.set_sap_payment_defaults()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
