-- Replace the raw requests.status filter with a computed lifecycle_status
...
create or replace function get_request_tracking(
  ...
  p_status text default null,           -- 'All' | 'pending_mr' | 'pending_bid_entry' | 'pending_po' | 'pending_all' | 'open_order'
...
comment on function get_request_tracking is
  'Backs the Request Tracking screen. lifecycle_status is computed (rejected/cancelled/closed_order/open_order/pending_po/pending_bid_entry/pending_mr); p_status also accepts the virtual value pending_all (groups the three pending_* states). Tenant-scoped, not role-restricted.';
